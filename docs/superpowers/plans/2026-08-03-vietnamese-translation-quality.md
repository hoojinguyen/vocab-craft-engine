...
- **Post-Task-4 final review (2026-08-03):** Step 2 ingestion (`main.py:456`) still did `or def_item["definition_en"]`, re-writing the English gloss that Task 3 removed from `kaikki_parser` — success criterion #4 violated. Fixed: fallback dropped + `test_extract_fields_definition_vi_none_without_vietnamese_translation` added.
- **Post-Task-4 final review (2026-08-03):** Spec §6/§7 rate-limit backoff was unimplemented. Added `backoff_seconds` (default 0.5s) to `Translator.__init__` (sleep between retry attempts) + `VI_BATCH_SLEEP_SECONDS = 0.1` inter-batch sleep in `_backfill`; backoff test added.
- **Post-Task-4 final review (2026-08-03):** Deferred (not merge-blockers): (a) spec §3.3 tier-2 (words in collocations/phrases/reflex_drills) and explicit MT-budget gating not implemented — step translates all NULL rows in graded-first order; checkpoint keeps re-runs cheap and budget can be simulated by `force_reset` + partial runs. (b) `fetchall()` materializes all missing definitions in memory (~150-250MB at ~1M rows) — survivable; keyset pagination on `(cefr_is_null, id)` is the follow-up. (c) Spec §8's `tests/test_nlp.py` extension implemented as a separate `tests/test_translator.py` (better isolation).
