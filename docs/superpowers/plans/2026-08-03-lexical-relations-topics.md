...
| 4 | Checkpoint: count-only (relations > 50k AND topics > 1k) | Added `AND existing_inverse > 0` completeness condition + repair test | Count-only checkpoint can permanently skip an incomplete inverse pass (crash between natural pass and inverse pass) — same regression 4G was fixed away from. Owner: Task 4. |
| 5 | Docs: "~30 curated themes" (spec §2, §3.2) | Documented "18 curated themes" | THEME_MAP actually defines 18 distinct themes; docs must match shipped reality. Owner: Task 6. |